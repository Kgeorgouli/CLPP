function [mappedX, mapping] = LPP(W,sG, options, data,balance)   %[eigvector, eigvalue] konstantia
...
%     balance=1;
    ...
    G = G ./ mG;
    sG = sG ./ mG;
    %Convert from distance to probability   
    % Compute Gaussian kernel (heat kernel-based weights)
    G(G ~= 0) = exp(-G(G ~= 0));
    sG(sG ~= 0) = exp(-sG(sG ~= 0));    
    
    if balance == -1;  %balance between graphs
        sd=eig(sLP);
        d=eig(LP);
        balance1 = sum(d)/sum(sd)
        balance2 = max(d)/max(sd)


        balance3 = sum(diag(cov(LP)))/sum(diag(cov(sLP)))
        balance4 = max(diag(cov(LP)))/max(diag(cov(sLP)))
...
%         W = (W.*tmpD_mhalf).*tmpD_mhalf'; %konstantia
        clear tmpD_mhalf;
    else
    ...
    data = repmat(DToPowerHalf,1,nFea).*data;% !! attention Konstantia
    sampleMeanscal = mean(data);
    [eigvector, eigvalue] = LGE(W, [], options, data);

%     [eigvector, eigvalue] = LGE(W, DToPowerHalf, options, data);
...
eigvector(:,eigIdx) = [];
%%Konstantia
mappedX = initialdata * eigvector;
mapping.M = eigvector;
mapping.mean = sampleMean;  %%%sampleMean right

if ~isfield(options,'Regu') || ~options.Regu
    mapping.DToPowerHalf=DToPowerHalf;
end
mapping.nSmp=nSmp;
