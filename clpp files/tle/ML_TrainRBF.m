function [B,C,sigma] = ML_TrainRBF(X,Y,Nc,rbfType,clusterType,C)
%rbf - 0 thin plate, 1 - gaussian sigma==1, 2 - gaussian, 3 - real gaussian
dbstop if error
...
if ~exist('C', 'var') || isempty(C)
    if ~exist('clusterType', 'var') || isempty(clusterType)
        ...
        if clusterType==0
            ...
            %EM
            %estS = gmmb_em(X, 'components', Nc, 'thr', 1e-30,'maxloops',300);
...
for i=1:N
    ...
    for j=1:Nc
        if (rbfType == 3)
            ...
            %H(i,j)= norm(X(i,:) - C(j,:)./sqrt(sigma(j)));
        elseif (rbfType == 4)
